    ...
    async fn Scale(&self,functionName: &str,namespace: &str)->FunctionScaleResult{
        ...
        if res.replicas == 0 {
            ...
            let mut routine = |attempt: u64|async move {
                ...
                attempt, self.Config.SetScaleRetries, functionName, minReplicas));
                match setResult {
            ...
            for i in 1..self.Config.SetScaleRetries{
                ...
                match res{
                    ...
                    ScalingError::HttpError(a,b )=>{
                        println!("Scale fail:{}/{},error:httperror{},{}",i,self.Config.SetScaleRetries,a,b);
                        let scale_result = ScalingError::HttpError((a), (b));
                    }
                    ScalingError::InvalidFactor(a)=>{
                        println!("Scale fail:{}/{},error:invalid factor{}",i,self.Config.SetScaleRetries,a);
                        let scale_result = ScalingError::InvalidFactor((a));
                    }
                    ScalingError::JsonError(a)=>{
                        println!("Scale fail:{}/{},error:json error{}",i,self.Config.SetScaleRetries,a);
                        let scale_result = ScalingError::JsonError((a));
                    }
                    ScalingError::LabelParse(a)=>{
                        println!("Scale fail:{}/{},error:label parse{}",i,self.Config.SetScaleRetries,a);
